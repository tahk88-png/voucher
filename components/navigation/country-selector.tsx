...
import { useCountry, Country } from "@/components/contexts/country-context"
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          aria-label={`Select market (${selectedCountry.code})`}
        >
        ...
        {isOpen && (
          <div
            id={menuId}
            className="absolute top-full mt-2 right-0 w-64 bg-white rounded-[14px] shadow-warm border border-[rgba(139,115,85,0.1)] py-2 z-50 max-h-[420px] overflow-y-auto"
            role="listbox"
          >
    ...
    <WarmCard padding="lg">
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          aria-expanded={isOpen}
          aria-controls={menuId}
          aria-label={`Select market (${selectedCountry.name})`}
        >
          <div className="flex items-center gap-3">
        ...
        {isOpen && (
          <div
            id={menuId}
            className="absolute top-full mt-2 left-0 right-0 bg-white rounded-[12px] shadow-warm border border-[rgba(139,115,85,0.1)] py-2 z-50 max-h-[400px] overflow-y-auto"
            role="listbox"
          >
            {availableCountries.map((country) => (
              <button
