          ...
          return (
            <WarmCard key={index} hover padding="lg">
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`w-12 h-12 rounded-[14px] bg-gradient-to-br ${stat.color} flex items-center justify-center shadow-warm`}
                >
              ...
              <div className="text-2xl font-bold text-[#2D2721] mb-1">
                {(typeof stat.value === "number" &&
                  stat.label.includes("Earned")) ||
                stat.label.includes("Month") ? (
                  <CurrencyDisplay
              ...
              {filteredTransactions.map((txn) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-[#FFF9ED] flex items-center justify-center flex-shrink-0">
                          <Icon
                            className={`h-4 w-4 ${getTransactionColor(txn.type, txn.amount)}`}
                          />
                        </div>
                        <div>
                          <div className="font-medium text-[#2D2721]">
                            {txn.description}
                          </div>
                          {txn.referenceId && (
                    ...
                    <td className="py-4 px-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getStatusBadge(txn.status)}`}
                      >
                        {txn.status}
                      </span>
                    </td>
                    <td
                      className={`py-4 px-4 text-right font-semibold ${getTransactionColor(txn.type, txn.amount)}`}
                    >
                      {txn.amount > 0 ? "+" : ""}
                      <CurrencyDisplay
                        amount={txn.amount}
              ...
              <div key={txn.id} className="p-4 bg-[#FFF9ED] rounded-xl">
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center flex-shrink-0">
                    <Icon
                      className={`h-5 w-5 ${getTransactionColor(txn.type, txn.amount)}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-[#2D2721] mb-1">
                      {txn.description}
                    </div>
                    <div className="text-xs text-[#8B7355]">{txn.date}</div>
                    {txn.referenceId && (
                      <div className="text-xs text-[#8B7355] font-mono mt-1">
                        {txn.referenceId}
                      </div>
                    )}
                  </div>
                  <div className="text-right">
                    <div
                      className={`text-lg font-semibold ${getTransactionColor(txn.type, txn.amount)}`}
                    >
                      {txn.amount > 0 ? "+" : ""}
                      <CurrencyDisplay
                        amount={txn.amount}
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${getStatusBadge(txn.status)}`}
                    >
                      {txn.status}
                    </span>
                  </div>
