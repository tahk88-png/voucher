...
function NavButton({ active, onClick, icon: Icon, label }: any) {
  return (
    ...
    >
      <Icon
        className={`w-4 h-4 ${active ? "text-[#E17B5C]" : "text-[#8B7355]"}`}
      />
      {label}
    </button>
  );
...
function IntegrationsTab() {
  const [integrations, setIntegrations] = useState([
    {
      ...
      active: true,
      icon: CreditCard,
      desc: "Makselingid ja pangatehingute import.",
    },
    {
      id: "lhv",
      name: "LHV Pank",
    ...
    },
    {
      ...
      active: false,
      icon: Truck,
      desc: "Pakkide saatmine SmartPosti kaudu.",
    },
...

    {
      ...
      active: false,
      icon: Instagram,
      desc: "Kuva oma viimaseid postitusi poe avalehel.",
    },
    {
      id: "fb_shop",
      name: "Facebook Shop",
    ...
    },
    {
      ...
      active: false,
      icon: Youtube,
      desc: "Lisa tootelehtedele videoid otse oma kanalist.",
    },
  ]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {groups.map((group) => (
        <div key={group.type}>
          <h3 className="text-sm font-bold text-[#8B7355] uppercase tracking-wider mb-4 border-b border-[#E7DCC7] pb-2">
            {group.title}
          </h3>
          <div className="grid md:grid-cols-2 gap-4">
            {integrations
              ...
              .map((item) => (
                <div
                  key={item.id}
                  className={`p-5 rounded-xl border-2 transition-all flex items-start gap-4 ${item.active ? "bg-white border-[#00D098]/30 shadow-sm" : "bg-[#FAF7F2] border-transparent"}`}
                >
                  <div
                    className={`p-3 rounded-lg ${item.active ? "bg-[#E6F4EA] text-[#00D098]" : "bg-[#E7DCC7] text-[#6B5744]"}`}
                  >
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="p-2 hover:bg-[#E7DCC7]/30 rounded text-[#8B7355] transition-colors"
                      title="Kopeeri"
                    >
                      <Copy className="w-4 h-4" />
                    </button>
                    <button
                      className="p-2 hover:bg-red-50 rounded text-[#8B7355] hover:text-red-500 transition-colors"
                      title="Kustuta"
                    >
      ...
      <div className="bg-[#2D2721] text-[#E7DCC7] p-6 rounded-xl flex items-start gap-4">
        ...
        <div>
          ...
          <WarmButton
            variant="outline"
            className="border-[#E7DCC7]/20 text-white hover:bg-white/10 hover:text-white"
          >
            Seadista Webhooke
          </WarmButton>
        </div>
      ...
      <div className="bg-white p-8 rounded-xl border border-[#E7DCC7] shadow-sm">
        ...
        <div className="bg-[#FAF7F2] rounded-lg p-6 border border-[#E7DCC7]">
          <h4 className="font-bold text-[#2D2721] mb-4 flex items-center gap-2">
            <div
              className={`w-2.5 h-2.5 rounded-full ${status === "active" ? "bg-[#00D098]" : "bg-[#FFC857] animate-pulse"}`}
            ></div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-white border border-[#E7DCC7] rounded text-sm">
              ...
              <div className="flex items-center gap-4">
                <span className="font-mono text-[#6B5744]">
                  cname.gifthub.ee
                </span>
                {status === "active" ? (
                  <Check className="w-4 h-4 text-[#00D098]" />
                ) : (
                  <RefreshCw className="w-4 h-4 text-[#FFC857] animate-spin" />
                )}
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl border border-[#E7DCC7] divide-y divide-[#E7DCC7] shadow-sm">
        ...
        <div className="p-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-[#2D2721]">EUR</span>
          </div>
        </div>
        ...
        <div className="p-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs bg-[#E17B5C] text-white px-2 py-1 rounded">
              PRO
            </span>
            <Switch />
          </div>
