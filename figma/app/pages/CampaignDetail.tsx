...
export function CampaignDetail() {
      ...
      if (found) {
        ...
        setCampaign({
          ...
          merchantDetails: {
            ...
            mapImage:
              "https://images.unsplash.com/photo-1569336415962-a4bd9f69cd83?ixlib=rb-4.1.0&auto=format&fit=crop&w=800&q=80",
          ...
          longDescription:
            "See on suurepärane võimalus nautida kvaliteetset teenust. Pakkumine kehtib piiratud aja jooksul.",
        });
      }
      setLoading(false);
    }
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#FAF7F2] flex items-center justify-center">
        <Loader2 className="animate-spin text-[#E17B5C]" />
      </div>
    );
  if (!campaign)
    return (
  ...
  const discount = campaign.original_price
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-[#E7DCC7] sticky top-16 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center gap-2 text-sm text-[#8B7355]">
          <button
            ...
            className="hover:text-[#2D2721] flex items-center gap-1"
          >
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-[#2D2721] font-medium truncate max-w-[200px]">
            {campaign.title}
          </span>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {galleryImages.map((img, i) => (
                <div
                  key={i}
                  className="rounded-xl overflow-hidden aspect-[4/3] shadow-sm cursor-pointer hover:shadow-warm transition-all"
                >
                  <ImageWithFallback
                    src={img}
                    alt="Gallery"
          ...
          <div className="space-y-6 motion-safe:animate-fade-up">
            <WarmCard padding="lg" className="sticky top-24 bg-white">
              <div
                onClick={() =>
                  campaign.merchantId &&
                  navigate(`/merchant/${campaign.merchantId}`)
                }
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-[#2D2721] group-hover:text-[#E17B5C] transition-colors">
                    {campaign.merchantDetails.name}
                  </h3>
              ...
              <div className="mb-8">
                ...
                {campaign.original_price && (
                  <span className="text-sm font-bold text-[#E17B5C] bg-[#FFF9ED] px-2 py-1 rounded-md">
                    Säästad{" "}
                    {Math.round(
                      (1 - campaign.price / campaign.original_price) * 100,
                    )}
                    %
                  </span>
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <section>
              <h2 className="text-2xl font-bold text-[#2D2721] mb-4">
                Pakkumise sisu
              </h2>
            ...
            <section id="vouchers" className="scroll-mt-24">
              ...
              <div className="space-y-4">
                ...
                <WarmCard
                  padding="lg"
                  hover
                  className="bg-white border-l-4 border-l-[#E17B5C]"
                >
                ...
                <WarmCard
                  padding="lg"
                  hover
                  className="bg-white border-l-4 border-l-[#9DB5A5]"
                >
                  <div className="flex flex-col sm:flex-row gap-6">
                    <div className="flex-1">
                      <div className="flex justify-between">
                        ...
                        <span className="bg-[#9DB5A5] text-white text-xs font-bold px-2 py-1 rounded h-fit">
                          TASUTA
                        </span>
                      </div>
                      <p className="text-[#6B5744] mb-4">
                        Kasuta seda koodi broneerimisel või kohapeal.
                      </p>

                      <div className="bg-[#FAF7F2] p-3 rounded-lg border border-dashed border-[#8B7355]/30 flex items-center justify-between">
                        <span className="font-mono font-bold text-[#2D2721] tracking-widest">
                          {freeCode}
                        </span>
                        <button
                          onClick={() => handleCopyCode(freeCode)}
                          className="text-[#E17B5C] hover:text-[#D16B4C]"
                        >
